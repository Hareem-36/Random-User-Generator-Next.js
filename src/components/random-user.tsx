...
const RandomUser: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-yellow-200 to-yellow-400">
      <h1 className="text-4xl font-bold mb-4 bg-slate-50 text-green-600">Random User Generator</h1>
      <p className="text-muted-foreground mb-6 font-bold text-zinc-800">
        Click the button below to fetch a random user profile.
      </p>
      <Button onClick={fetchRandomUser} className="mb-6 bg-blue-800 font-bold">
        Fetch New User
      </Button>
      {loading && (
      ...
      {user && (
       <Card className="border-2 border-blue-800 shadow-lg rounded-lg overflow-hidden max-w-sm relative">

          <CardHeader className="h-32 bg-lime-700 relative">
            <Image
              src={user.image}
              alt={user.name}
              width={80}
          ...
          <CardContent className="p-6 pt-12 text-center">
            <CardTitle className="text-xl font-bold flex items-center justify-center text-emerald-800">
              <UserIcon className="mr-2" /> {user.name}
            </CardTitle>
            <CardDescription className="text-muted-foreground flex items-center justify-center text-blue-800 font-bold">
              <MailIcon className="mr-2" /> {user.email}
            </CardDescription>
            <div className="text-sm text-muted-foreground mt-2 flex items-center justify-center text-blue-800 font-bold">
            ...
            <Button
              variant="outline"
              className="mt-4 bg-blue-800 text-zinc-50 font-bold"
      ...
      <br></br>  <div className="text-center text-green-600 font-bold bg-slate-50">
